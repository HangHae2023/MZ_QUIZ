...
isIDDuple = async (userId) => {
  ...
  return res.status(200).send({ userId, success:true, message:"사용가능한 아이디입니다" });
...
isNicknameDuple = async (nickname) => {
  ...
  return res.status(200).send({ nickname, success:true, message:"사용가능한 닉네임입니다"  });
  ...
  userSignup = async (userId, password, nickname) => {
    ...
    const user = await this.signupRepository.userSignup(userId, hashedPassword, nickname);
  
    return ({success:true, message:'회원가입에 성공했습니다'});
  } catch(err) {throw new ValidationError('예상못한 에러')}
}
